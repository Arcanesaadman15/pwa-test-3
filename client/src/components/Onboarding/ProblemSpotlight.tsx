  ...
  return (
    <div className="h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black overflow-hidden flex flex-col">
      ...
      <div className="absolute top-4 right-4 z-20 safe-area-top">
        <button
          onClick={onComplete}
          className="px-3 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-gray-300 hover:text-white hover:bg-white/20 transition-all duration-200 text-sm font-medium"
        >
          Skip
        </button>
      </div>
      ...
      >
            ...
            <motion.div
              ...
              >
                <div className={`w-20 h-20 rounded-3xl bg-gradient-to-br ${PROBLEM_SPOTLIGHT_SLIDES[currentSlide].gradient} 
                               flex items-center justify-center shadow-2xl backdrop-blur-sm border border-white/10`}>
                  <span className="text-3xl">{PROBLEM_SPOTLIGHT_SLIDES[currentSlide].image}</span>
                </div>
                
                {/* Subtle glow */}
                <div className={`absolute inset-0 rounded-3xl bg-gradient-to-br ${PROBLEM_SPOTLIGHT_SLIDES[currentSlide].gradient} 
                               opacity-20 blur-xl -z-10 scale-110`} />
            </motion.div>
              ...
              <motion.div
                ...
                className="max-w-sm mx-auto bg-white/5 backdrop-blur-xl rounded-3xl p-5 border border-white/10 shadow-2xl"
              >
                {/* Problem Title */}
            ...
            >
              <Button
                ...
                className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 
                         text-white font-bold py-4 text-lg rounded-xl shadow-2xl hover:shadow-3xl 
        ...
        {currentSlide === 0 && (
          ...
          >
            <div className="flex items-center space-x-3 text-gray-500 text-sm bg-white/5 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
        ...
        {!isLastSlide && (
          <button
            ...
            className="absolute right-4 top-1/2 transform -translate-y-1/2 w-12 h-12 
                     bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center
                     text-white hover:scale-110 active:scale-95 transition-all duration-200 border border-white/20
                     shadow-xl z-10"
          >
            →
          </button>
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <motion.div
            className="absolute top-1/4 -right-32 w-64 h-64 bg-blue-500/3 rounded-full blur-3xl"
            animate={{
              scale: [1, 1.2, 1],
              opacity: [0.3, 0.6, 0.3],
            }}
          ...
          />
          <motion.div
            className="absolute bottom-1/4 -left-32 w-64 h-64 bg-purple-500/3 rounded-full blur-3xl"
            animate={{
              scale: [1.2, 1, 1.2],
              opacity: [0.3, 0.6, 0.3],
            }}
