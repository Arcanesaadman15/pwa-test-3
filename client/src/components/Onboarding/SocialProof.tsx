  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white">
      <div className="container mx-auto px-4 py-8 max-w-md">
        ...
        >
          {Object.entries(SUCCESS_STATS).map(([key, value], index) => (
            <div key={key} className="text-center bg-white/10 rounded-xl p-4 backdrop-blur-lg">
              <div className="text-2xl font-bold text-blue-400">{value}</div>
              <div className="text-xs text-gray-300 mt-1">
                {key === 'totalUsers' && 'Active Users'}
                {key === 'avgTestosteroneIncrease' && 'T-Level Boost'}
                {key === 'completionRate' && 'Success Rate'}
              </div>
            </div>
          ))}
        </motion.div>
